...
class Config(object):
  ...
  def __init__(self , file=None  , dir=None , base_dir = None):
    ...
    if dir:
      ...
      logger.debug("Missing config file or directory - setting dir to current working dir")
      dir = os.getcwd()
    
    # Set attributes      
    self.cmdvVersion = cfg['cmdvVersion'] if "cmdvVersion" in cfg else None   # v1.0
    self.Class       = cfg['class'] if "class" in cfg else None               # CMDVGlobalConfig
    self.label       = cfg['label'] if "label" in cfg else "Global-Config"    # Global-Config
    self.doc         = cfg['doc']   if "doc"   in cfg else None               # Template for global config
    ...
    if dir and os.path.exists(dir) :
      ...
      if not "type" in cfg['repo'] :
        logger.warning("Missing type field for repo - assuming local directory")
      ...
      if cfg['repo']['type'] == "local" :
        ...
        if "path" in cfg['repo'] and "type" in cfg['repo'] :
          if os.path.isabs(cfg['repo']['path']) :
            logger.warning("Repo path is absolute can make problems.")
            loc['path'] = cfg['repo']['path']   
          else :
            # make aboslute for current machine and in relation to config file
            loc['path'] = cfg['self']['path'] + "/" + cfg['repo']['path']   
        ...
        logger.error("Unknown type " + cfg['repo']['type'] + " - Not implemented")  
        sys.exit(1)      
    else:
    ...
    if "session" in cfg :      
      ...
      self.session = Session(
                              ...
                              name=None , 
                              prefix=None , 
                              path= base_dir if base_dir and os.path.exists(base_dir) else os.getcwd() ,
                              )
  ...
  def _find_config(self, repo_dir=None ,  config_name = "cmdv-testing.config.yaml" , repo_type=None) :  
    ...
    if not config_file:
      logger.debug("Searching for config - following directory structure down - starting at " +str(repo_dir))
