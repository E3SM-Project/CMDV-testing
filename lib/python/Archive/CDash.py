...
# class CDash:
  ...
  def push(self, directory=None , report=None ) :
    ...
    f.write( "Test session: " + (report.name if report.name else "default" )+ " (" + report.date + ")\n")
    f.write( "Number test runs: " + str( len(report.tests)) + "\n" )
    f.write( "Report: \n")
    for test in report.tests :
      f.write (  test.name + ":\t" + test.status if test.status else "unknown \n")
      for step in test.steps :
        f.write( ":".join([step.name , (step.status if step.status else "success")]) + "\t" )
      f.write("\n")
    f.close()
