...
class Tool(object):
    ...
    def _assign_values(self, inputs):
        ...
        if self.inputs:
            ...
            logger.warning(
                "Can't assign values to inputs. No tool inputs specified")
                ...
                if isinstance(self.inputs[k], dict):
                    ...
                    if 'inputBinding' in i:
                        ...
                        if 'position' in i['inputBinding'] and i['inputBinding']['position']:
                            options_positional.append(
                                {"value": option, 'position': i['inputBinding']['position']})
                        else:
                    ...
                    logger.error(
                        "Not Implemented - Argumemt type not determined ")
                    sys.exit(1)
        ...
        if cfg:
            ...
            if 'baseCommand' in cfg['run']:
                if isinstance(cfg['run']['baseCommand'], list):
                    ...
                    logger.error("Unsupported type for run: " +
                                 type(cfg['run']['baseCommand']))
                    sys.exit(1)
                ...
                logger.error(
                    "Can't initialize step, missing baseCommand. Tools or Workflows not implemented")
                sys.exit(1)
