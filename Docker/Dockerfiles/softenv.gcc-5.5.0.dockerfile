...
# wget http://mirrors-usa.go-parts.com/gcc/releases/gcc-5.5.0/gcc-5.5.0.tar.gz
RUN wget http://mirrors.concertpass.com/gcc/releases/gcc-5.5.0/gcc-5.5.0.tar.gz && \
  tar -xf gcc-5.5.0.tar.gz && \
  mkdir -p /gcc && \
  mkdir tmp && \
  cd tmp && \
  /Downloads/gcc-5.5.0/configure \
  --prefix /gcc \
