...
! Data
!
  integer, public :: ncol_for_qsat = pver
...
! note that this uses ncol_for_qsat, which is a box model kludge
   ...
   real(r8), intent(in) :: t(ncol_for_qsat,*)    ! Temperature
   real(r8), intent(in) :: p(ncol_for_qsat,*)    ! Pressure
   real(r8), intent(out) :: es(ncol_for_qsat,*)   ! Saturation vapor pressure
   real(r8), intent(out) :: qs(ncol_for_qsat,*)   ! Saturation specific humidity

!ubroutine aqsat(t       ,p       ,es      ,qs        ,ii      , &
!                ilen             ,kk      ,kstart  ,kend      )
   call    aqsat(t       ,p       ,es      ,qs      ,ncol_for_qsat, &
                 ncol_for_qsat    ,pver    ,1       ,pver      )
   return
end subroutine qsat
